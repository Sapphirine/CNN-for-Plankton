...
from lasagne.layers import InputLayer, DenseLayer, get_output,MaxPool2DLayer,Conv2DLayer,Layer,DropoutLayer
...
p_dropout = theano.shared(np.cast['float32'](0.5))
l9_5 = DropoutLayer(l9,p = p_dropout)
l10 = cyclicpool(l9_5)
...
l2_penalty = regularize_layer_params([l7,l5,l4,l3,l9,l11,l1], lasagne.regularization.l2)*np.cast['float32'](.00001)
...
#PRED = []
i= 28548
rate.set_value(np.cast['float32'](0.0002))
...
while i <=100000:
    ...
    if cycle:
        ...
        LOSS.append(newloss)
        if len(LOSS) >2:
            if (LOSS[-1]-LOSS[-2])>1.5 or np.isnan(LOSS[-1]) :
                print 'diverge!!'
                saveparam(para0,i)
                setpvalue(para0,updates.keys())
    ...
    if i%(renewtrain/4) ==0 and i!=0:
        ...
        ratio = 0.5
        if len(LOSS)>2500:
            ratio = (-np.mean(np.array(LOSS[len(LOSS)-1000:len(LOSS)]))+np.mean(np.array(LOSS[len(LOSS)-2000:len(LOSS)-1000])))/np.mean(np.array(LOSS[len(LOSS)-1000:len(LOSS)-500]))
        ...
        random.shuffle(temp)
        p_dropout.set_value(np.cast['float32'](0.))
        ...
        p_dropout.set_value(np.cast['float32'](0.5))
